...
public static class BootBringUp
{
    ...
    private static bool WaitHttp(UtmHttpClient http, int port, TimeSpan timeout, string? expectedFsrar, Action<string> log)
    {
        ...
        while (DateTime.UtcNow < deadline)
        {
            ...
            // RSA невалиден, но ГОСТ читается → токен сел, УТМ фактически поднят. Это типовое
            // состояние после перевыпуска КЭП (RSA перевыпускается через сам УТМ). Считаем
            // поднятым — иначе bring-up/heal бесполезно гоняли бы работающий УТМ. Личность
            // токена гарантирована самой хореографией (introduce нужного ридера / peel-down по серийнику).
            if (info is not null && info.GostValid)
            {
                log($"  порт {port}: RSA невалиден, но ГОСТ valid — токен сел, считаю поднятым (нужен перевыпуск RSA через УТМ)");
                return true;
            }
            Thread.Sleep(1500);
        }
